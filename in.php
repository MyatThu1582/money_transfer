      ...
      $category_id = $_SESSION['category_id'];
        ...
        if($_POST){
          ...
          if(!empty($balancedatas)){
            $balance = intval($balancedatas['balance']) + intval($amount);
          }else{
            if (!empty($opening_balancedatas)) {
              $balance = intval($opening_balancedatas['opening_amt']) + intval($amount);
            }else{
              $balance = $amount;
            }
          }
          ...
          $stmt = $pdo->prepare("INSERT INTO cashbook (date,description,in_amt,balance,category_id) VALUES ('$date','$des','$amount','$balance','$category_id')");
          $query = $stmt->execute();

          
          $cash_idstmt = $pdo->prepare("SELECT * FROM cashbook WHERE date='$date' AND category_id='$category_id' ORDER BY id DESC");
          $cash_idstmt->execute();
      ...
      <div class="container mt-5 w-50">
        <div class="card">
          <div class="card-header pt-3 pb-3">
            <h3>ငွေသွင်းမှတ်တမ်းထည့်သွင်းရန်</h3>
          </div>
          <div class="card-body">
